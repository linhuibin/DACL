...
class DIFEX(Algorithm):
    ...
    def teanettrain(self, dataloaders, epochs, opt1, sch1):
        ...
        for epoch in range(epochs):
            ...
            all_z = torch.angle(torch.fft.fftn(all_x, dim=(-2, -1)))
            ...
            self.scaler.step(
                opt1)  # 首先把梯度值unscale回来，如果梯度值不是inf或NaN,则调用optimizer.step()来更新权重，否则，忽略step调用，从而保证权重不更新。　　 scaler.step(optimizer)
    ...
    def update(self, minibatches, opt, sch):
        ...
        with autocast(enabled=self.amp):
            with torch.no_grad():
                all_x1 = torch.angle(torch.fft.fftn(all_x, dim=(-2, -1)))
